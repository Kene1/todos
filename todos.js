...
function checkIfUserExists(email, password) {
  for (const user of database) {
    if (
      ...
      user.user.password === hashCode(password)
    ) {
      hideAllErrors();
      loginUser(user);
      return;
  ...
  showError("login-errors", "Incorrect username and/or password. ");
}
    ...
    <div id="dashboard">
      ...
      <ul id="dashboard-todo-lists">
        ${loggedInUser.lists
          .map(
            (list) =>
              `<li><a href="#">${list.title}</a> (${list.items.length})
  ...
  <div id="list">
    ...
    <ul id="todo-list-items">
      ${selectedListItems
        .map(
          (listItem) =>
            `<li class="${listItem.done ? "done" : ""}">${listItem.text}</li>`
        )
        .join("")}
    </ul>
...
function addEventListenersApp() {
  ...
  if (document.getElementById("signup-form")) {
    document
      ...
      .addEventListener("submit", signupFormSubmitted);
  }

  if (document.getElementById("login-form")) {
    document
      .getElementById("login-form")
      .addEventListener("submit", loginFormSubmitted);
  }
  ...
  if (document.getElementById("settings-button")) {
    document
      ...
      .addEventListener("click", settingsButtonClicked);
  }

  if (document.getElementById("dashboard-button")) {
    document
      .getElementById("dashboard-button")
      .addEventListener("click", dashboardButtonClicked);
  }
  ...
  if (document.getElementById("dashboard")) {
    if (document.getElementById("dashboard").querySelector("ul")) {
      document
        ...
        .querySelector("ul")
        .addEventListener("click", displayListDetailScreen);
    }
  }
  ...
  if (document.getElementById("list")) {
    if (document.getElementById("list").querySelector("ul")) {
      document
        .getElementById("list")
        .querySelector("ul")
        .addEventListener("click", changeDoneStatusOfItems);
    }
  }
  ...
  if (document.getElementById("create-new-list")) {
    document
      ...
      .addEventListener("click", createNewListButtonClicked);
  }

  if (document.getElementById("new-item-form")) {
    document
      .getElementById("new-item-form")
      .addEventListener("submit", newItemFormSubmitted);
  }

  if (document.getElementById("change-title")) {
    document
      .getElementById("change-title")
      .addEventListener("submit", changeTitleFormSubmitted);
  }

  if (document.getElementById("settings-form")) {
    document
      .getElementById("settings-form")
      .addEventListener("submit", settingsFormSubmitted);
  }
}
...
function signupFormSubmitted(e) {
  ...
  const confirmSignupPassword =
    targetForm.querySelector("#confirm-password").value;
  ...
  if (signupPassword !== confirmSignupPassword) {
    signupError += " Passwords must match";
    errors = true;
  }
...
function newItemFormSubmitted(e) {
  ...
  if (listTitle === "New list") {
    showError(
      "new-item-errors",
      "First you have to add a custom title to your list."
    );
    return;
  }
  ...
  // const datePicker = <input type="date" id="date-picker" />;
  // const dueDate = datePicker.getElementById("date-picker").value;

  const newLI = document.createElement("li");
  newLI.innerText = newValue;
  document.getElementById("list").querySelector("ul").appendChild(newLI);
  // document.getElementById("list").querySelector("ul").appendChild(dueDate);
